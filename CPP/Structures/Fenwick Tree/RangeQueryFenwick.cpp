...
class RangeQueryFenwick {
  ...
  public:
	RangeQueryFenwick(int N_, const T& t, const F& f, bool indexing_ = 0): N(N_ + 1), func(f), indexing(indexing_) {
		A.resize(N + 1, t);
		initializer = t;
		BIT1.resize(N + 1, t);
		BIT2.resize(N + 1, t);
	...
	RangeQueryFenwick(const vector<T>& V, const T& t, const F& f, bool indexing_ = 0): func(f), indexing(indexing_) {
