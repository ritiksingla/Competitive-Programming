...
template<typename T>
class LinearDiophantine :  public LinearEquation<T> {
  protected:
	using LinearEquation<T>::A;
	using LinearEquation<T>::B;
	using LinearEquation<T>::gcd;
	using LinearEquation<T>::extended_gcd;
  public:
	T C;
	LinearDiophantine(T A_, T B_, T C_): LinearEquation<T>(A_, B_) {
		C = C_;
	}
